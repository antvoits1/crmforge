function renderModal() {
  ...
  if (m.type === "file") {
    ...
    ov.innerHTML = `<div class="modal document-viewer">
      ...
      <div class="scan"><div class="scan-paper">
        <div class="scan-brand">${esc(lead().bank.name.toUpperCase())}</div>
        <div class="row-between scan-meta"><div>${esc(lead().company)}<br>${esc(lead().bank.acct)}</div><div>${esc(f.n)} · ${esc(f.p)}</div></div>
        <p>Scanned file preview. ${esc(f.n)} for ${esc(lead().company)}.</p>
