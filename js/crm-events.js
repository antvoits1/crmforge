document.addEventListener("click", (e) => {
  ...
  if (!b) {
    ...
    if (state.searchOpen && !e.target.closest("#crmSearchPop")) {
      state.searchOpen = false; state.query = ""; renderSearch();
    }
    return;
  }
  ...
  if (act === "search-toggle") { state.searchOpen = !state.searchOpen; if (!state.searchOpen) state.query = ""; renderSearch(); if (state.searchOpen) setTimeout(()=>$("q")?.focus(), 0); return; }
  if (act === "search-result") {
    state.selected = b.dataset.lead;
    state.searchOpen = false; state.query = "";
  ...
  if (act === "dtmf") {
    ...
    if (state.dial.status === "idle") {
      ...
      state.dial.dtmf += k;
      const timer = $("timer"); if (timer) timer.textContent = state.dial.dtmf;
      toast("DTMF " + k);
    }
    return;
  }
...
document.addEventListener("keydown", (e) => {
  if (e.key === "/" && document.body.dataset.page === "leads" && !/INPUT|TEXTAREA/.test(document.activeElement.tagName) && document.activeElement.isContentEditable !== true) {
    e.preventDefault(); state.searchOpen = true; renderSearch(); setTimeout(()=>$("q")?.focus(), 0);
  }
  if (e.key === "Escape") {
